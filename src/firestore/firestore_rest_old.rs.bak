...
use crate::firestore::types::{
    DocumentReference, DocumentSnapshot, MapValue, Settings, SnapshotMetadata,
};
...
impl Firestore {
    ...
    pub async fn get_document(
        ...
        // Deserialize directly into protobuf Document type
        use crate::firestore::types::proto::google::firestore::v1::Document;
        let doc: Document = response.json().await?;

        // Extract fields from Document into MapValue
        let data = if !doc.fields.is_empty() {
            Some(
                crate::firestore::types::proto::google::firestore::v1::MapValue {
                    fields: doc.fields,
                },
            )
        } else {
    ...
    pub async fn set_document(
        &self,
        path: impl AsRef<str>,
        data: crate::firestore::types::MapValue,
        ...
        // Create Document with fields from MapValue
        use crate::firestore::types::proto::google::firestore::v1::Document;
        let doc = Document {
            name: format!(
    ...
    pub async fn update_document(
        ...
        let update_mask: Vec<&str> = data.fields.keys().map(|k| k.as_str()).collect();
    ...
    > {
        ...
        let stream = async_stream::stream! {
            ...
            while !cancelled.load(std::sync::atomic::Ordering::SeqCst) {
                ...
                use crate::firestore::types::proto::google::firestore::v1::Document;
                let doc: Document = match response.json().await {
                    Err(e) => {
                ...
                // Check if document changed (use update_time from Document)
                let update_time = doc.update_time.as_ref().map(|ts| format!("{}_{}", ts.seconds, ts.nanos));
                if last_update_time.as_ref() != update_time.as_ref() {
                    last_update_time = update_time;

                    let data = if !doc.fields.is_empty() {
                        Some(crate::firestore::types::proto::google::firestore::v1::MapValue {
                            fields: doc.fields,
                        })
                    } else {
                        None
        ...
        let writes: Vec<serde_json::Value> = batch
            ...
            .map(|op| match op {
                WriteOperation::Set { path, data } => {
                    ...
                    let fields_json = crate::firestore::conversion::map_to_firestore_fields(data);
                    serde_json::json!({
                        "update": {
                            "name": full_path,
                            "fields": fields_json
                ...
                WriteOperation::Update { path, data } => {
                    ...
                    let update_mask: Vec<String> = data.fields.keys().cloned().collect();
                    let fields_json = crate::firestore::conversion::map_to_firestore_fields(data);
                    serde_json::json!({
                        "update": {
                            "name": full_path,
            ...
            FilterCondition::Equal(field, val) => {
                serde_json::json!({
                    "fieldFilter": {
                        ...
                        "op": "EQUAL",
                        "value": crate::firestore::conversion::value_to_firestore_json(val)
                    }
                })
            }
            ...
            FilterCondition::ArrayContainsAny(field, vals) => {
                serde_json::json!({
                    "fieldFilter": {
                        ...
                        "value": {
                            "arrayValue": {
                                "values": vals.iter().map(|v| crate::firestore::conversion::value_to_firestore_json(v)).collect::<Vec<_>>()
                            }
                        }
                    }
                })
    ...
    async fn execute_query(&self, query: Query) -> Result<Vec<DocumentSnapshot>, FirebaseError> {
        ...
        if let Some(start_values) = &query.start_at {
            structured_query["startAt"] = serde_json::json!({
                "values": start_values.iter().map(|v| crate::firestore::conversion::value_to_firestore_json(v)).collect::<Vec<_>>(),
                "before": false
            });
        }
        ...
        if let Some(end_values) = &query.end_at {
            structured_query["endAt"] = serde_json::json!({
                "values": end_values.iter().map(|v| crate::firestore::conversion::value_to_firestore_json(v)).collect::<Vec<_>>(),
                "before": false
            });
        }
        ...
        let documents: Vec<DocumentSnapshot> = results
            ...
            .filter_map(|result| {
                result.get("document").and_then(|doc| {
                    ...
                    // Convert Firestore fields format to MapValue
                    let data = fields.and_then(|f| {
                        crate::firestore::conversion::firestore_fields_to_map(f).ok()
                    });
...
pub struct Query {
    ...
    start_at: Option<Vec<crate::firestore::types::Value>>,
    start_after: Option<Vec<crate::firestore::types::Value>>,
    end_at: Option<Vec<crate::firestore::types::Value>>,
    end_before: Option<Vec<crate::firestore::types::Value>>,
}
...
impl Query {
    ...
    /// # }
    /// ```
    pub fn start_at(mut self, values: Vec<crate::firestore::types::Value>) -> Self {
    ...
    /// # }
    /// ```
    pub fn start_after(mut self, values: Vec<crate::firestore::types::Value>) -> Self {
    ...
    /// # }
    /// ```
    pub fn end_at(mut self, values: Vec<crate::firestore::types::Value>) -> Self {
    ...
    /// # }
    /// ```
    pub fn end_before(mut self, values: Vec<crate::firestore::types::Value>) -> Self {
...
mod tests {
    ...
    async fn test_query_builder() {
        ...
        let query = fs
            .collection("users")
            .query()
            .where_filter(FilterCondition::Equal(
                "name".to_string(),
                crate::firestore::conversion::json_to_value(&json!("Alice")),
            ))
            .order_by("age", OrderDirection::Ascending)
            .limit(10);
